...
### API Endpoints
- `GET /api/exercise-templates` - List exercise templates for user
- `POST /api/exercise-templates` - Create exercise template
- `DELETE /api/exercise-templates/:id` - Delete exercise template
...
3. **Bottom Navigation**: 5-tab bottom hotbar (Pläne, Übungen, Training, Statistik, Profil) with center Training tab elevated/larger
4. **Active Training Flow**: Per-set checkmarks with auto-advance, last-performance preview, workout logs persisted to DB via `workout_logs` table
